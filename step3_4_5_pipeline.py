...
# Lớp ghi log song song ra màn hình và file kết quả
...
# Hàm tính toán chỉ số KS (Kolmogorov-Smirnov)
...
def main():
    # Khởi tạo 2 thư mục riêng biệt để lưu kết quả và biểu đồ
    results_dir = 'results'
    plots_dir = 'plots'
    os.makedirs(results_dir, exist_ok=True)
    os.makedirs(plots_dir, exist_ok=True)
    
    # Tạo mã thời gian (timestamp) để phân biệt các lần chạy
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    log_file_path = os.path.join(results_dir, f"results_{timestamp}.txt")
    img_name = os.path.join(plots_dir, f"feature_importance_{timestamp}.png")
    img_shap_name = os.path.join(plots_dir, f"shap_summary_{timestamp}.png")
    ...
    # 1. Đọc dữ liệu đã được gộp từ Bước 1
    input_file = os.path.join('data_output', 'cleaned_data.csv')
    if not os.path.exists(input_file):
        print(f"Lỗi: Không tìm thấy file dữ liệu tại '{input_file}'. Vui lòng chạy step1_2_data_processing.py trước.")
        return
        
    print(f"Bắt đầu chạy thử nghiệm cấp cao (Q1 Journal Standard) lúc: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("Đang đọc dữ liệu từ data_output/cleaned_data.csv...")
    df = pd.read_csv(input_file)
    
    # 2. Tiền xử lý dữ liệu & Tạo đặc trưng cải tiến (Bước 2 nâng cao)
    ...
    # Tạo các biến tỷ lệ tài chính dựa trên kiến thức ngành (Domain Knowledge)
    print("Tạo các biến tỷ lệ tài chính (DIR, AIR, ACR, DAR)...")
    ...
    # Tạo các biến tương tác từ các nguồn điểm tín dụng bên thứ ba (Interaction Features)
    ...
    # === BƯỚC 3: HUẤN LUYỆN CHÉO (3-FOLD CROSS-VALIDATION) & SO SÁNH 4 MÔ HÌNH ===
    print("\n" + "="*80)
    print("BƯỚC 3: HUẤN LUYỆN CHÉO (3-FOLD STRATIFIED CV) & SO SÁNH 4 THUẬT TOÁN")
    print("="*80)
    
    # Định nghĩa 4 thuật toán so sánh với siêu tham số tối ưu
    models = {
    ...
    # Tính trị trung bình và độ lệch chuẩn (Mean ± SD) cho từng mô hình
    print("\n" + "="*80)
    ...
    # Mô hình tốt nhất dựa trên Mean AUC
    best_model_name = max(summary_auc, key=summary_auc.get)
    print(f"Mô hình tối ưu nhất được lựa chọn: {best_model_name}")
    
    # Kiểm định ý nghĩa thống kê (Paired t-test) giữa mô hình tốt nhất và các mô hình còn lại
    ...
    # Phân tách tập dữ liệu 80/20 cố định để tạo mô hình cải tiến & chạy SHAP
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=0.20, random_state=42, stratify=y
    )
    
    # Huấn luyện mô hình cơ sở tốt nhất trên tập train 80%
    ...
    # === BƯỚC 4: TRIỂN KHAI ĐIỂM CẢI TIẾN ===
    print("\n" + "="*80)
    print("BƯỚC 4: TRIỂN KHAI ĐIỂM CẢI TIẾN (scale_pos_weight = 11.5 / class_weight)")
    print("="*80)
    ...
    if best_model_name == "LightGBM":
        ...
        # CatBoost dùng auto_class_weights hoặc scale_pos_weight
    ...
    # === BƯỚC 5: GIẢI THÍCH MÔ HÌNH (XAI) BẰNG SHAP & FEATURE IMPORTANCE ===
    print("\n" + "="*80)
    print("BƯỚC 5: GIẢI THÍCH MÔ HÌNH BẰNG SHAP & FEATURE IMPORTANCE")
    print("="*80)
    
    # 5.1. Trích xuất đặc trưng quan trọng
    ...
    # 5.2. Tính toán SHAP Values phục vụ giải thích cục bộ & toàn cục (Local & Global)
    print("\nĐang tính toán SHAP values trên mẫu đại diện (500 khách hàng)...")
    try:
        explainer = shap.TreeExplainer(improved_model)
        # Lấy mẫu ngẫu nhiên đại diện 500 dòng từ X_val để tính toán nhanh
        ...
        plt.title(f"SHAP Summary Plot - {best_model_name} Improved (Q1 Standard)", fontsize=14, pad=20)
        plt.tight_layout()
    ...
    print(f"\nĐã lưu trữ toàn bộ số liệu và cơ sở chứng minh khoa học tại: {os.path.abspath(log_file_path)}")
    print(f"Đã lưu biểu đồ đặc trưng quan trọng tại: {os.path.abspath(img_name)}")
